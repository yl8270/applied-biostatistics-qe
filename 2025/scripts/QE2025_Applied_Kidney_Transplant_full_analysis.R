...
tidy_cox_hr <- function(model, model_label = NULL) {
  broom::tidy(model, exponentiate = TRUE, conf.int = TRUE) %>%
    ...
    select(model, term, HR = estimate, conf.low, conf.high, HR_95_CI, p.value, p_value)
}

# Safe Cox fitting function for repeated univariate and interaction models.
safe_coxph <- function(formula, data) {
  tryCatch(
    survival::coxph(formula, data = data, x = TRUE, y = TRUE),
    error = function(e) {
      message("Model failed: ", deparse(formula), " | ", e$message)
      return(NULL)
    },
    warning = function(w) {
      message("Model warning: ", deparse(formula), " | ", w$message)
...
table1_gts <- surv_dat %>%
  select(all_of(c("alg_use", table1_vars))) %>%
  tbl_summary(
    by = alg_use,
    label = var_labels,
    type = list(all_continuous() ~ "continuous2"),
    statistic = list(
      all_continuous() ~ c("{median} ({p25}, {p75})", "{mean} ({sd})"),
      all_categorical() ~ "{n} ({p}%)"
    ),
...
univ_results <- purrr::imap_dfr(univ_models, ~ tidy_cox_hr(.x, model_label = .y)) %>%
  ...
  select(Variable, term, `Hazard ratio (95% CI)`, `P value`)
...
full_cox <- survival::coxph(full_formula, data = surv_dat, x = TRUE, y = TRUE)
...
try({
  ...
  print(survminer::ggforest(selected_cox, data = surv_dat, fontsize = 0.8))
  grDevices::dev.off()
}, silent = TRUE)
...
martingale_df <- surv_dat %>%
  mutate(martingale_resid = residuals(full_cox, type = "martingale"))
...
fit_interaction_lrt <- function(modifier) {
  ...
  reduced_model <- safe_coxph(reduced_formula, surv_dat)
  interaction_model <- safe_coxph(interaction_formula, surv_dat)
...
interaction_ft <- interaction_screen %>%
  ...
  select(Modifier, LRT_Chisq, df, `P value`) %>%
  flextable::flextable() %>%
...
for (mod in selected_modifiers_for_detail) {
  ...
  m_int <- safe_coxph(f_int, surv_dat)
  if (!is.null(m_int)) {
...
sensitivity_cox <- survival::coxph(sensitivity_formula, data = surv_dat, x = TRUE, y = TRUE)
