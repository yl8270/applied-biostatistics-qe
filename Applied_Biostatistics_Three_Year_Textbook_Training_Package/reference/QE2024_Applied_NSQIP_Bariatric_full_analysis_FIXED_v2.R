...
# IMPORTANT FIX:
# Some versions of gtsummary are strict about label objects for many derived
# complication variables. To avoid label parsing errors, Table 3 is built
# manually here. This also gives us complete control over n (%), p-values, and
# readable labels.
...
# Helper for chi-square/Fisher p-values. We use chi-square by default for
# speed. If expected cell counts are small, Fisher's exact test with simulated
# p-value avoids FEXACT workspace errors.
safe_binary_table_p <- function(x, group) {
  tab <- table(x, group, useNA = "no")
  if (nrow(tab) < 2 || ncol(tab) < 2) return(NA_real_)
  expected <- suppressWarnings(chisq.test(tab, correct = FALSE)$expected)
  if (any(expected < 5)) {
    return(suppressWarnings(fisher.test(tab, simulate.p.value = TRUE, B = 10000)$p.value))
  }
  suppressWarnings(chisq.test(tab, correct = FALSE)$p.value)
}

make_comp_row <- function(v) {
  raw_name <- toupper(sub("_bin$", "", v))
  label_value <- unname(individual_comp_labels[[raw_name]])
  if (is.null(label_value) || is.na(label_value) || length(label_value) != 1) label_value <- v

  d <- dat %>%
    dplyr::select(procedure, value = dplyr::all_of(v)) %>%
    dplyr::filter(!is.na(procedure), !is.na(value))

  group_levels <- levels(dat$procedure)
  if (is.null(group_levels)) group_levels <- sort(unique(as.character(d$procedure)))

  out <- tibble::tibble(Complication = label_value)
  for (g in group_levels) {
    denom <- sum(d$procedure == g, na.rm = TRUE)
    n_yes <- sum(d$procedure == g & d$value == 1, na.rm = TRUE)
    out[[g]] <- fmt_n_pct(n_yes, denom)
  }
  out$`P value` <- fmt_p(safe_binary_table_p(d$value, d$procedure))
  out
}

table3_manual <- purrr::map_dfr(complication_bin_vars, make_comp_row)
write_csv_safely(table3_manual, file.path(OUT_TABLES, "00_table3_individual_complications.csv"))

table3_ft <- table3_manual %>%
  flextable::flextable() %>%
  make_three_line(font_size = 7)
...
# IMPORTANT FIX:
# Some versions of gtsummary do not allow exponentiate = TRUE for lm objects.
# Because the model is lm(log(LOS) ~ predictors), exponentiating coefficients
# is exactly how we interpret them as ratios of geometric mean LOS. We therefore
# build this table manually from broom::tidy().
los_results <- broom::tidy(los_model, conf.int = TRUE) %>%
  dplyr::mutate(
    Ratio = exp(estimate),
    conf.low.ratio = exp(conf.low),
    conf.high.ratio = exp(conf.high),
    `Ratio of geometric mean LOS (95% CI)` = paste0(
      round(Ratio, 3), " (", round(conf.low.ratio, 3), ", ", round(conf.high.ratio, 3), ")"
    ),
    `P value` = fmt_p(p.value),
    Term = dplyr::case_when(
      term == "(Intercept)" ~ "Intercept",
      term == "procedureGastric Bypass" ~ "Procedure type: Gastric Bypass vs Gastric Banding",
      term == "age_years" ~ "Age, years",
      term == "bmi" ~ "Body mass index, kg/m^2",
      term == "sexFemale" ~ "Sex: Female vs Male",
      stringr::str_detect(term, "race_ethnicity") ~ stringr::str_replace(term, "race_ethnicity", "Race/ethnicity: "),
      term == "smoking_statusYes" ~ "Current smoker: Yes vs No",
      term == "asa_severitySevere" ~ "ASA severity: Severe vs Mild",
      TRUE ~ term
    )
  ) %>%
  dplyr::filter(term != "(Intercept)") %>%
  dplyr::select(Term, `Ratio of geometric mean LOS (95% CI)`, `P value`)

los_ft <- los_results %>%
  flextable::flextable() %>%
  make_three_line(font_size = 8)
write_csv_safely(los_results, file.path(OUT_TABLES, "06_los_loglinear_model.csv"))
